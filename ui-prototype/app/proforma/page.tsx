...
import { computeProForma, money, PRO_FORMA_DEFAULTS } from "@/lib/financials";

export default function ProFormaPage() {
  const [rent, setRent] = useState(PRO_FORMA_DEFAULTS.rentPerUnitMonth);
  const [psf, setPsf] = useState(PRO_FORMA_DEFAULTS.hardCostPerGsf);
  const [cap, setCap] = useState(PRO_FORMA_DEFAULTS.exitCapRatePct);
  const acquisitionPrice = PRO_FORMA_DEFAULTS.acquisitionPrice;
          ...
          <div className="pg-grid-aside400">
            <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
              ...
              <div style={{ background: "var(--panel)", border: "1px solid var(--line)", borderRadius: 8, overflow: "hidden" }}>
                ...
                <LineItemRow
                  label="Buildable area — FAR 0.7"
                  value={`${envelope.buildableGsf.toLocaleString("en-US")} GSF`}
                  badge={{ tone: "orange", label: "OFFICIAL · UNVERIFIED" }}
                />
                <LineItemRow
                  label="Height / coverage caps"
                  value={`${envelope.maxHeightFt} ft · ${subjectParcel.maxLotCoveragePct}%`}
                  badge={{ tone: "orange", label: "OFFICIAL · UNVERIFIED" }}
                  last
                />
              </div>
